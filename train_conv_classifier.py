...
def swap_axes_data(input_data):
    ...
    swapped_data = np.swapaxes(input_data, 1, num_axis - 1)
...
if __name__ == '__main__':
    ...
    conv_feature = LAGFeatureExtraction(compute_lag=extract, lag=10)
    ...
    for cross_valid_it in np.arange(held_out_obs.shape[1]):
        ...
        conv = ConvClassifier(batch_size=FLAGS['batch_size'], learning_rate=FLAGS['learning_rate'],
                              ...
                              input_dim=[None, train_data[0].shape[1], train_data[0].shape[2], train_data[0].shape[3]],
                              batch_norm=True,
